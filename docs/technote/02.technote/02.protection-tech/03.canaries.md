...
* Terminator Canaries의 값은 NULL (0x00), CR (0x0d), LF (0x0a) 및 EOF (0xff)로 구성되어 있습니다.
  + 공격자는 Canaries를 우회하기 위해 위해 Return addresss를 쓰기 전에 null 문자를 써야 합니다.
...
* Random Canaries는 프로그램 초기 설정 시에 전역 변수에 Canary 값이 저장된다.
  ...
  + 공격자가 Canary 값이 저장된 stack addresss를 알거나 스택의 값을 읽어올수 있는 프로그램이 있다면 Canary의 값을 확인 할 수 있습니다.
...
* **다음과 같은 방법으로 바이너리의 Canary 설정여부를 확인합니다.**
  ...
  + 파일의 심볼 테이블에 "\_\_stack\_chk\_fail"가 있으면 Canary가 적용되었다고 판단합니다.
